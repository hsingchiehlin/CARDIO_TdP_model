...
parameters {
	...
	real<lower =-3> m_Emax;	// max effect size
...
transformed parameters {
	...
	for (i in 1:Ni) { // un-log transform
		y0[i] = exp(m_y0);
		x0[i] = exp(m_x0);
		Emax[i] = inv_logit(m_Emax); // Emax between 0 and 1
		n[i] = exp(m_n);
	}
}
model {
...
//	real ec10_GSD_approx;		// Approximate GSD derived from m_0,m_1,sd_0, and sd_1
	...
	for (i in 1:Ni) {
		if (Emax[i] > 1) 
			ec99[i] = x0[i]*(0.99 * Emax[i] / (Emax[i] - 0.99))^(1/n[i]);
		else
			ec99[i] = 1000;
	}
	for (i in 1:Ni) {
		if (Emax[i] > 0.95)
			ec95[i] = x0[i]*(0.95 * Emax[i] / (Emax[i] - 0.95))^(1/n[i]);
		else	
			ec95[i] = 1000;
	}
	for (i in 1:Ni) {

...
POD.ALL.endpoints <- as.data.frame(matrix(NA, 138, 5))
colnames(POD.ALL.endpoints) <- endpoint.list
...
for(i in c(1:3)){
  ...
  for(j in 1:1050){
    ...
    all.mcmc.dat <- read.csv(paste(chempath,"/", "C", chemical.num,"_standat_v7_do_stan_fit_", tmp.endpoint, "_MCMC_Samples.csv", sep = ""))
    
    temp.mcmc.dat <- all.mcmc.dat[, which(colnames(all.mcmc.dat) == "ec05.1")]

    POD.ALL.endpoints[j,i] <- quantile(temp.mcmc.dat, probs = 0.5)

    rownames(POD.ALL.endpoints) <- chemmap$Chemical.Name

    write.csv(POD.ALL.endpoints, paste(getwd(), "/", "POD_study_design_A.CSV", sep = ""), row.names = T)

    print(j)
  }
  print(endpoint.list[i])
}
...
for(i in c(4)){
  ...
  for(j in c(1:1050)){
    ...
    temp.mcmc.dat <- all.mcmc.dat[, which(colnames(all.mcmc.dat) == "ec95.1")]
...
for(i in c(5)){
  ...
  for(j in c(1:1050)){
    ...
    temp.mcmc.dat <- all.mcmc.dat[, which(colnames(all.mcmc.dat) == "ec10.1")]
